        ...
        <div class="card-body">
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <a href="{{ route('cetak_pdf', $siswa->nisn) }}" class="btn btn-success mb-4">Cetak Raport</a>
                ...
                <div class="tab-pane fade show active" id="belum" role="tabpanel">
                    ...
                    @if ($mapel_belum->isEmpty())
                        <div class="alert alert-warning" role="alert">
                            Data Mapel Sudah Dinilai Semua
                        </div>
                    @else
                                ...
                                <tbody>
                                    @foreach ($mapel_belum as $item)
                                        <tr>
                                            ...
                                            <td>{{ $item->semester }}</td>
                                            <td>
                                                <a href="{{ route('halaman_nilai', ['id_siswa' => $siswa->nisn, 'id_mapel' => $item->kd_mapel]) }}"
                                                    class="btn btn-primary">Nilai</a>
                                            </td>
                                ...
                                <tbody>
                                    @foreach ($mapel_sudah as $item)
                                        <tr>
                                            ...
                                            <td>{{ $item->detail_kelas->kelas->kelas }}</td>
                                            <td>{{ $item->semester }}</td>
                                            <td>{{ $item->nilai_akhir }}</td>
                                            <td>
                                                <a href="{{ route('halaman_ubah_nilai', $item->id) }}"
                                                    class="btn btn-warning">Ubah Nilai</a>
                                                    <a href=" {{ route('halaman_detail_nilai', $item->id) }}" class="btn btn-primary">Detail Nilai</a>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
